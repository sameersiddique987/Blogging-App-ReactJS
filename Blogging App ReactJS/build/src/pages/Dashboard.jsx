...
console.log(uid);
  ...
  const editbtn = (index) => {
    ...
    const updatedDescription = prompt("Updated Description");
    ...
    if (updatedTitle || updatedDescription) {
      const updatedBlogs = blogs.map((blog, i) => {
        if (i === index) {
          return {
            ...blog, 
            title: updatedTitle || blog.title, 
            description: updatedDescription || blog.description, 
          };
        }
        return blog;
      });
...
<form  onSubmit={handleSubmit(formValue)}   className="  t-5 bg-white px-10 mx-44 border border-none shadow-lg  rounded p-10">
